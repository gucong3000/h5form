...
//				eventInvalid.fire(oEvent);
		...
		setHolder = (function(){
			...
			function setDisplay(){
				clearTimeout(timer);
				if(holder){
					var show = holder.innerHTML && !elem.value && /^text(area)?|password|email|search|tel|url$/i.test(elem.type),
						currStyle = currentStyle(elem),
						style = runtimeStyle(holder),
						parent = elem.parentNode,
						disp = parent && (elem.offsetHeight || elem.offsetWidth);
					...
					if(!disp){
						...
						timer = setTimeoutFn(function(){
							...
							if(parent === elem.offsetParent && !doc.querySelector && /^normal$/i.test(currentStyle(parent).zoom)) {
